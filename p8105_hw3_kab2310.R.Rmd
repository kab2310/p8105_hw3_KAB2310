...
#overall 
citi_summary_stats <- citibikes_cleaned |>
  summarise(
    total_rides = n(),
    avg_duration = mean(duration),
    max_duration = max(duration),
    min_duration = min(duration),
    total_members = sum(member_casual == "member"),
    total_casual = sum(member_casual == "casual")
  )

glimpse(citibikes_cleaned)
citi_summary_stats
```
The cleaned citi bike dataset contains 99,253 rides, with the average rude duration being 13.8. The longest recorded ride duration was 239 minutes. The 
minimum duration was1 minute. 79,711 rides were taken by Citi Bike members and 19,542 rides taken by casual riders.

#the total number of rides in each combination of year and month separating casual riders and Citi Bike members.
```{r}
citibikes_cleaned <- citibikes_cleaned |>
  mutate(year = case_when(
    str_detect(ride_id, "2020") ~ "2020",
    str_detect(ride_id, "2024") ~ "2024"
  )) |>
  mutate(month = case_when(
    str_detect(ride_id, "Jan") ~ "January",
    str_detect(ride_id, "July") ~ "July"
  ))